        ...
        <div className="max-w-[min(90vw,1760px)] mx-auto grid grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)] gap-10 lg:gap-14">
          ...
          <section className="mb-20" id="modes">
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-white/10 bg-white/2 p-6">
                ...
                <p className="text-zinc-400 text-sm leading-relaxed mb-3">
                  You need <code className="bg-white/10 px-1.5 py-0.5 rounded text-zinc-300">GOOGLE_CLOUD_PROJECT</code> and <code className="bg-white/10 px-1.5 py-0.5 rounded text-zinc-300">GOOGLE_CLOUD_LOCATION</code> in <code className="bg-white/10 px-1.5 py-0.5 rounded text-zinc-300">.env.local</code>, plus Vertex AI credentials (<code className="bg-white/10 px-1.5 py-0.5 rounded text-zinc-300">GOOGLE_SERVICE_ACCOUNT_JSON_B64</code>, or ambient ADC for local dev). Veo produces clips of roughly 8 seconds. For longer videos (e.g. 30 or 60 seconds), the system splits the script into ~8-second chunks, generates one Veo clip per chunk, then concatenates them with <strong className="text-zinc-300">ffmpeg</strong>. So for talking_object videos longer than 8 seconds, ffmpeg must be installed (on PATH or via <code className="bg-white/10 px-1.5 py-0.5 rounded text-zinc-300">FFMPEG_PATH</code>). If the script is short for the requested duration, the system may extend the script so the video fills the length.
                </p>
          ...
          <section className="mb-20" id="env">
            ...
            <div className="rounded-xl border border-white/10 bg-white/2 p-6 font-mono text-sm space-y-3 text-zinc-400">
              ...
              <div><span className="text-amber-400/80">GOOGLE_CLOUD_PROJECT</span>, <span className="text-amber-400/80">GOOGLE_CLOUD_LOCATION</span> - Vertex AI project and region for Veo (e.g. us-central1). The app refuses to boot without both.</div>
